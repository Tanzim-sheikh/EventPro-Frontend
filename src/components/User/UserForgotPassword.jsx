...
const UserForgotPassword = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.log('User forgot-password error:', error?.response?.status, error?.response?.data);
  ...
  const handleReset = async (e) => {
    ...
    try {
      ...
      console.log('User reset-password error:', error?.response?.status, error?.response?.data);
      setErrorMsg(error.response?.data?.message || 'Something went wrong.');
    } finally {
      setIsLoading(false);
    }
        ...
        <div className="mt-8 sm:mx-auto sm:w-full sm:max-w-md border-[#8C9F6E] border-2 rounded-lg shadow-lg shadow-[#8C9F6E]">
          <div className="bg-[#f0f5e8] py-8 px-4 sm:rounded-lg sm:px-10">
            ...
            <p className="mt-6 text-center text-sm text-gray-600">
              ...
              <Link to="/auth/user/login" className="font-medium text-[#A3B886] hover:text-[#7a8a5f]">
                Login
              </Link>
            </p>
          </div>
